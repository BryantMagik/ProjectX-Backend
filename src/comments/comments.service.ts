...
export class CommentsService {
  ...
  async create(createCommentDto: CreateCommentDto, user: UserActiveInterface) {
    ...
    if (taskId && issueId) {
      throw new BadRequestException(
        'Solo puede especificar taskId o issueId, no ambos',
      );
    }
  ...
  async getCommentsByUser(user: UserActiveInterface) {
    if (!user || !user.id) {
      throw new UnauthorizedException(
        'Usuario no autenticado o ID de usuario faltante.',
      );
    }
    ...
    if (!comments || comments.length === 0) {
      throw new NotFoundException(
        'No se encontraron comentarios para el usuario especificado.',
      );
    }
  ...
  async findCommentsByTask(taskId: string) {
    ...
    if (!comments || comments.length === 0) {
      throw new NotFoundException(
        `No se encontraron comentarios para la tarea con ID ${taskId}`,
      );
    }
  ...
  async findCommentsByIssue(issueId: string) {
    ...
    if (!comments || comments.length === 0) {
      throw new NotFoundException(
        `No se encontraron comentarios para el issue con ID ${issueId}`,
      );
    }
